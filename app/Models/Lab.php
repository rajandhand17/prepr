    ...
    {  
        try { 
           // DB::beginTransaction();
            ...
            $labSlug = $labSlug_format = str_slug($request->title);
            $next = 1;
            ...
            if ($request->file('image')) {
                ...
                Storage::disk('s3')->put('uploads/labs/' . $filename, $image);
            ...
            $tags = (!empty($request->lab_tags)) ? json_encode($request->lab_tags) : $request->lab_tags;
            ...
            $lab->member_type         = $request->radi_online;
            $lab->member              = $request->member;
            $lab->country             = $request->country;
            $lab->latitute            = $request->city_lat;
            $lab->longitude           = $request->city_lng;
            ...
            DB::rollBack();
            return redirect()->route('userlab.index')->with(['error' => __('notification.notification_swwptal')]);
        } catch (\Exception $e) {
            DB::rollBack();
            abort(500);
        }
