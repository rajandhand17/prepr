...
def write_to_env_file(secret, file_path=".env.beta"):
    try:
...
def delete_encrypted_file(file_path=".env.beta.encrypted"):
    try:
...
def encrypt_env_file(file_path=".env.beta"):
    try:
        print(f"Encrypting {file_path}...")
        result = subprocess.run(["php", "artisan", "env:encrypt", "--env=beta"], capture_output=True, text=True)
...
def set_vapor_key(output):
    try:
        ...
        with open(".env.beta", "w") as f:
            f.write(f'LARAVEL_ENV_ENCRYPTION_KEY={key}\n')
        expect_script = """
        spawn vapor env:push beta --file=.env.beta
        ...
        if result.returncode != 0:
            print(f"Error pushing .env.beta to Vapor: {result.stderr}")
        else:
            print(result.stdout)
    except Exception as e:
...
def main():
    secret_name = "learnlab/Serverless/Beta"
